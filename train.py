    ...
    with tf.Session() as sess:
        ...
        for step in range(settings.TRAIN_STEPS):
            ...
            if step % 50 == 0:
                ...
                # 保存图片
                imageio.imwrite('{}-{}.jpg'.format(settings.OUTPUT_IMAGE, step), img)
                # tf.train.Saver().save(sess, save_path='//savemodel//mode')
                # imageio.imwrite('images/output/{}-{}.jpg'.format(settings.OUTPUT_IMAGE,step), img)
        tf.train.Saver().save(sess, save_path=settings.savemodepath)
        # 保存最终训练结果
        ...
        # imageio.imwrite('images/output/{}.jpg'.format(settings.OUTPUT_IMAGE), img)
        imageio.imwrite('{}.jpg'.format(settings.OUTPUT_IMAGE), img)
